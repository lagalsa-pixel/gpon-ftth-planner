#!/usr/bin/env python3
"""Отрисовка FTTH-сетей v2 на космоснимках Google Satellite z18 (кэш тайлов).

Отличия v2: абоненты пронумерованы по схеме «ОРШ.№» (12.7 = ОРШ 12, абонент 7);
доп. абоненты (добавленные до требуемого числа по таблице СНП ВКО) показаны
круглыми маркерами рядом с выявленными домохозяйствами; сплиттеры
промаркированы как ОРШ; легенда дополнена параметрами сверки и нумерации."""
...
from ftth_render import (fetch_tile, download_tiles, lon2mx, lat2my,
                         load_font, star_points, CACHE,
                         C_TRUNK, C_TRUNK_CASE, C_DIST, C_DIST_CASE, C_DROP,
                         C_HH, C_HH_FILL, C_SPL, C_POP, C_AERIAL)

OUT_DIR = '/home/z/my-project/download/snp_vko_ftth_v2'
...
def render_village2(d, idx):
    ...
    # подписи: 6 позиций вокруг точки, выбирается первая свободная (сетка занятости)
    CELL = 64
    _grid = {}
    ...
    # ---- 7. ОРШ (шкафы со сплиттерами GPON 1:32) ----
    f_spl = load_font(24)
    ...
    t3 = (f"Абоненты: {s['n_hh']} = выявлено {s['n_identified']} + добавлено {s['n_added']} "
          ...
          f"оптического кабеля: {s['total_km']:.1f} км")
    bar_h = 212
    ...
    rows = [
        ...
        ('line', C_DROP, 2, f"Дроповые линии к абонентам — {s['drops_km']:.2f} км"),
        ...
        ('text', None, 0, "Запас 7% на магистраль/распределение, 15 м на ввод в здание"),
    ]
    lw = 1560
    lhh = lh * len(rows) + 130
...
def main():
    ...
    for idx, key in items:
        d = json.load(open(os.path.join(DESIGN_DIR, f'{key}_design2.json')))
        render_village2(d, idx)
