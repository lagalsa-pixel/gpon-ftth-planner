# -*- coding: utf-8 -*-
"""
Шаг 47 (Task 47). Синхронизация захардкоженных констант в скриптах
пересборки (30/34/38) с итогами книги work/boq_decentral_data_v4.json,
пересобранной ПОСЛЕ полной VLM-верификации (52b) остатка кандидатов
(Винное/Солнечное/Алтайский). Запускать ПОСЛЕ 54_topology_v4.py и ДО
30/34/38.

OLD-константы — состояние Task 46 (частичная верификация):
  2644 ДХ, 55 сплиттеров, 1112 муфт, 4 320 портов, 1 429,4 км, 305 м.
Дополнительно: подпись детекции в 34 -> «полная VLM-верификация»,
с титула 38 убирается ярлык версии «v3:».
"""
import io
import json
...
OLD = dict(dhx=2644, zones=28, orsh=34, splitters=55, mufty=1112,
           splices=5939, ports=4320, fiber=1429.4, route=305)
...
pairs34 = [
    ...
    (f"уточнённая детекция ДХ, критерии Task 46): {OLD['zones']} зон + 6 ЦУ = {OLD['orsh']} ОРШ",
     f"уточнённая детекция ДХ, полная VLM-верификация): {NEW['zones']} зон + 6 ЦУ = {NEW['orsh']} ОРШ"),
...
pairs38 = [
    (f"{ru_num(OLD['dhx'])} обслуживаемых домохозяйств · {OLD['zones']} зонных ОРШ + 6 ЦУ · v3: ОРШ в центрах секторов, фидеры в границе НП",
