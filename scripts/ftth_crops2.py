#!/usr/bin/env python3
"""Кропы 1:1 из полноразмерных FTTH-снимков v2 для контроля качества VLM:
зона POP, самый загруженный ОРШ и участок с наибольшей плотностью доп. абонентов."""
...
def crop_around(img, cx, cy, w, h, path, q=88):
    ...
    y0 = max(0, min(H - h, int(cx - w / 2) if False else int(cy - h / 2)))
...
for key in keys:
    ...
    # 3) плотность доп. абонентов
    extras = [h for h in d['households'] if h.get('extra')]
    if extras:
        ky = 111132.0
        kx = 111320.0 * math.cos(math.radians(d['anchor'][0]))
        xy = np.array([[h['lat'] * ky, h['lon'] * kx] for h in extras])
        best_i, best_cnt = 0, -1
        for i in range(len(xy)):
            cnt = int((((xy - xy[i]) ** 2).sum(1) <= 150.0 ** 2).sum())
            if cnt > best_cnt:
                best_cnt, best_i = cnt, i
        ex, ey = PX(extras[best_i]['lat'], extras[best_i]['lon'])
        crop_around(img, ex, ey, 1600, 1000, os.path.join(CROPS, f'{key}_extras.jpg'))
        print(f"{key}: кропы POP, {s_best['id']} ({s_best['hh']} аб.), доп. абоненты "
              f"({best_cnt} в радиусе 150 м) из {png} {W}x{H}")
    else:
        print(f"{key}: кропы POP, {s_best['id']} ({s_best['hh']} аб.); доп. абонентов нет "
              f"из {png} {W}x{H}")
    img.close()
print('готово:', sorted(os.listdir(CROPS)))
