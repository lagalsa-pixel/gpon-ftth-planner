# -*- coding: utf-8 -*-
"""
Шаг 48. Рендер карт зон ОРШ (схема D, топология v3) на кадрах пользователя.

Отличия от шага 37 (v1-карты):
  - данные: work/boq_decentral_data_v3.json (врезы = cut_log v3, ОРШ = медианы
...
Выход: download/snp_vko/<NN>_<Село>_зоны_ОРШ_схема_D.jpg (замена v1-карт)
        work/qa/zone_maps_v3/<NN>_preview.png
Запуск: python3 48_zone_maps_v3.py <key>|all
"""
...
QAD = f'{BASE}/work/qa/zone_maps_v3'
...
DBOOK = json.load(open(f'{BASE}/work/boq_decentral_data_v3.json', encoding='utf-8'))
...
def render_village(key):
    ...
    mx = S(26)
    f1 = fit_font(dh_, f"с. {db['name']} — схема D (v3): зонные ОРШ в центрах секторов, фидеры — кратчайшие пути в границе НП",
                  S(46), W - 2 * mx)
    dh_.text((mx, S(12)),
             f"с. {db['name']} — схема D (v3): зонные ОРШ в центрах секторов, фидеры — кратчайшие пути в границе НП",
    ...
    ld.text((S(20), S(14)), 'ЗОНЫ ОРШ (схема D v3, S_MIN = 15)', font=fl_t, fill=(240, 240, 240))
    yy = S(58)
