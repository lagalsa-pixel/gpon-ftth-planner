...
TOTD = DBOOK['totals']
assert sum(v['dhx_served'] for v in DBOOK['villages']) == TOTD['dhx_served'] == 3089
assert abs(sum(v['fiber_km'] for v in DBOOK['villages']) - TOTD['fiber_km']) < 0.1
assert sum(z['houses'] for v in DBOOK['villages'] for z in v['zones']) == 3089
...
for i, (name, a, b, c, d, fmt, best) in enumerate(ROWS):
    ...
    if name == 'Волокно-км на ДХ, км':
        ...
        for col, L in zip(range(4, 8), 'DEFG'):
            ws.cell(row=r, column=col, value=f'={L}{fk_r}/3089')
