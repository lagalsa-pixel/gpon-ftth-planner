# -*- coding: utf-8 -*-
"""
Шаг 49. Числовой QA выпущенных материалов топологии v3.
...
Проверки:
 1) КАРТЫ (download/snp_vko, 6 шт): существование и размер; канва = база +
    ...
    нижнем углу; превью work/qa/zone_maps_v3 существует.
 2) КНИГА xlsx: лист «Децентрализация ОРШ» — число строк зон = ОРШ v3;
    лист «Сводная (схема D)» — итог волокно-км = totals v3.
 3) Итоговая сверка totals v3 (zones/orsh/fiber) с JSON.

Выход: консольный отчёт + work/qa/v3_report.json
"""
import json
import math
import os
...
print(f"\nкнига xlsx: строк зон в «Децентрализация ОРШ» = {n_zone_rows} "
      f"(книга v3: {tot['n_zones']})  {'OK' if xlsx_ok else '!! РАСХОЖДЕНИЕ'}")
if not xlsx_ok:
    report['ok'] = False
wb.close()

print(f"\nИТОГ QA v3: {'ВСЕ ПРОВЕРКИ ПРОЙДЕНЫ' if report['ok'] else 'ЕСТЬ ЗАМЕЧАНИЯ'}")
json.dump(report, open(f'{BASE}/work/qa/v3_report.json', 'w', encoding='utf-8'),
