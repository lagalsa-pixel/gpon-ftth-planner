# -*- coding: utf-8 -*-
"""
Шаг 16. Сводная таблица материалов FTTH по 6 СНП ВКО — КАСКАДНАЯ СХЕМА -> Excel.
Листы: «Сводная» (ВОР), «Параметры сетей», «Сравнение схем», «Методика».
...
D = json.load(open(f'{BASE_DIR}/work/boq_cascade_data.json', encoding='utf-8'))
...
base.setup_sheet(ws, title='Сводная таблица материалов — FTTH (GPON), каскадная схема, 6 СНП Восточно-Казахстанской области', last_col=LAST)
ws['B3'] = ('Каскадное деление 1:8 (ОРШ) × 1:8 (РОР) = 1:64 • сети — по спутниковым снимкам с привязкой к дорогам OSM, топология не менялась • '
            'допущения — лист «Методика» • сравнение с централизованной схемой — лист «Сравнение схем» • подготовлено ' + DATE)
ws['B3'].font = cap_font()
...
SECTIONS = [
    ('А. Оборудование и узлы', [
        ...
        ('Сплиттер PLC 1×8 — 1-я ступень (в ОРШ)', 'шт', 'сплиттеры 2-й ступ. / 8, окр. вверх', 'splitters1', 'int'),
        ('Сплиттер PLC 1×8 — 2-я ступень (в РОР)', 'шт', '1 на РОР (кластер до 8 ДХ)', 'splitters2', 'int'),
        ('РОР — бокс оптический распределительный (под сплиттер)', 'шт', '1 на кластер ДХ (радиус 100 м)', 'por_boxes', 'int'),
...
notes = [
    ...
    'Каскадная схема: сплиттеры 1-й ступени 1:8 — в ОРШ, 2-й ступени 1:8 — в РОР (суммарное деление 1:64). Нормы и допущения — лист «Методика»; параметры сетей — лист «Параметры сетей».',
]
for t in notes:
...
H2 = ['№', 'СНП', 'Район / с.о.', 'ДХ (заказ)', 'ДХ (проект)', 'Δ к заказу',
      'РОР', 'Сплиттеры 1:8, 2-я ступ.', 'Заполнение портов, %', 'Муфты (ветвл.)',
      'Магистраль, км', 'Волокно-км', 'Верхний участок, волокон',
      'Дропы проект, км', 'Подводки РОР→ДХ, км', 'Дроп-кабель, км',
      'Ср. дроп, м', 'Макс. дроп, м', 'ОРШ, портов', 'Сплиттеры 1:8, 1-я ступ.', 'Примечание']
L2 = 1 + len(H2)   # = 22 (V)
base.setup_sheet(ws2, title='Параметры FTTH-сетей по СНП — каскадная схема (1:8 в ОРШ + 1:8 в РОР)', last_col=L2)
ws2['B3'] = ('ДХ — домохозяйства; РОР — пункт распределительный оптический (бокс со сплиттером 1:8). «Верхний участок» — максимум волокон магистрали '
             'на одном участке (у ОРШ). Подготовлено ' + DATE)
...
H3 = ['№', 'Показатель', 'Ед. изм.', 'Централизованная (1:64 в ОРШ)', 'Каскадная (1:8 + 1:8)', 'Δ каскадной']
L3 = 1 + len(H3)   # = 7 (G)
base.setup_sheet(ws3, title='Сравнение схем деления: централизованная и каскадная', last_col=L3)
...
set_widths(ws3, {'A': 3, 'B': 5, 'C': 44, 'D': 9, 'E': 16, 'F': 16, 'G': 12})

cm, km_ = C['materials_total'], D['materials_total']
SIZES = [8, 12, 16, 24, 32, 48, 64, 72, 96]
cent_cable_km = round(sum(cm.get(f'cable_{s}', 0.0) for s in SIZES), 1)
casc_cable_km = round(sum(km_.get(f'cable_{s}', 0.0) for s in SIZES), 1)
...
COMP_ROWS = [
    ...
    ('Сплиттер PLC 1×64 (в ОРШ)', 'шт', cm.get('splitters'), None, 'int'),
    ('Сплиттер PLC 1×8, 1-я ступень (в ОРШ)', 'шт', None, km_.get('splitters1'), 'int'),
    ('Сплиттер PLC 1×8, 2-я ступень (в РОР)', 'шт', None, km_.get('splitters2'), 'int'),
    ('РОР — боксы распределительные', 'шт', None, km_.get('por_boxes'), 'int'),
    ('Муфты оптические', 'шт', cm.get('mufty'), km_.get('mufty'), 'int'),
    ('Пассивные узлы всего (муфты + РОР)', 'шт', cm.get('mufty'), round(km_.get('mufty', 0) + km_.get('por_boxes', 0)), 'int'),
    ('Порты PON OLT — справочно', 'шт', cm.get('olt_ports'), km_.get('olt_ports'), 'int'),
    ('Порты ОРШ, суммарно', 'шт', cent_orsh_ports, sum(v['orsh_ports'] for v in V), 'int'),
    ('Пигтейли SC/UPC для кросса ОРШ', 'шт', cm.get('pigtails'), km_.get('pigtails'), 'int'),
    ('S', 'Б. Кабельная продукция (с запасом на монтаж)', None, None, None, None),
]
for s in SIZES:
    COMP_ROWS.append((f'Кабель самонесущий, {s} волокон', 'км',
                      nz(cm.get(f'cable_{s}', 0.0)), nz(km_.get(f'cable_{s}', 0.0)), 'km'))
COMP_ROWS += [
    ('Магистральный кабель — всего', 'км', cent_cable_km, casc_cable_km, 'km'),
    ('Суммарная ёмкость магистрали', 'волокно-км', cent_fiber, D['totals']['fiber_km'], 'int'),
    ('Дроп-кабель 2-волоконный (с запасом 5 %)', 'км', cm.get('drop_cable_km'), km_.get('drop_cable_km'), 'km'),
    ('S', 'В. Монтаж и расходники', None, None, None, None),
    ('Сварные соединения', 'шт', cm.get('splices'), km_.get('splices'), 'int'),
    ('Коннекторы механические SC/UPC', 'шт', cm.get('fast_conn'), km_.get('fast_conn'), 'int'),
    ('Гильзы КДЗС, 60 мм', 'шт', cm.get('kdzs'), km_.get('kdzs'), 'int'),
    ('Комплекты подвеса магистрального кабеля', 'компл', cm.get('suspend_kits'), km_.get('suspend_kits'), 'int'),
    ('Комплекты подвеса дроп-подводок РОР→ДХ', 'компл', None, km_.get('drop_ext_suspends'), 'int'),
    ('Анкерные зажимы дроп-кабеля', 'шт', cm.get('drop_anchors'), km_.get('drop_anchors'), 'int'),
    ('Боксы абонентские оптические', 'шт', cm.get('abonent_boxes'), km_.get('abonent_boxes'), 'int'),
]
...
for item in COMP_ROWS:
    if item[0] == 'S':
        ...
        ws3.cell(row=r, column=6, value=kv)
        ws3.cell(row=r, column=7, value=f'=IF(AND(ISNUMBER(E{r}),ISNUMBER(F{r}),E{r}<>0),F{r}/E{r}-1,"—")')
        ...
        for c in range(2, L3 + 1):
            ...
            if c == 2:
                ...
                cell.alignment = Alignment(horizontal='right', vertical='center')
        ws3.cell(row=r, column=5).number_format = F_KM if kind == 'km' else F_INT
        ws3.cell(row=r, column=6).number_format = F_KM if kind == 'km' else F_INT
        ws3.cell(row=r, column=7).number_format = F_PCT
        ws3.row_dimensions[r].height = 22
    r += 1

r += 1
for t in ['Δ — отношение каскадной схемы к централизованной («—» — позиция отсутствует в одной из схем). Суммарный коэффициент деления одинаков: 1:64.',
          'Каскадная схема перераспределяет материалы: магистраль «легче» (волокно-км −71 %, кабели >48 волокон только у ОРШ крупных сёл), дроп-кабеля больше (+78 %), узлов меньше (муфты+РОР −45 %), сварок вдвое меньше.']:
...
L4 = 1 + len(H4)
base.setup_sheet(ws4, title='Методика расчёта и принятые допущения — каскадная схема', last_col=L4)
...
ROWS4 = [
    ...
    ('Геометрия сетей', 'Проектные FTTH-сети «ОРШ → муфты → ДХ»: построены по спутниковым снимкам высокого разрешения (~0,38 м/px) с привязкой к дорожной сети OSM; дропы проложены вдоль улиц, дворов и фасадов, все ветвления — в муфтах. При пересчёте под каскадную схему топология НЕ менялась.'),
    ...
    ('S', '2. Архитектура (каскадная схема)', None),
    ('Схема деления', 'GPON, двухступенчатое каскадное деление: сплиттер PLC 1×8 в ОРШ (1-я ступень) × сплиттер PLC 1×8 в РОР (2-я ступень) = 1:64. Суммарный коэффициент деления сохранён таким же, как в централизованной схеме (1:64); оптический бюджет — класс B+.'),
    ('РОР', 'Пункт распределительный оптический (бокс со сплиттером 1:8) размещается в точке магистрали (проектной муфте) и обслуживает кластер до 8 ДХ в радиусе 100 м по трассе сети. Всего 515 РОР на 6 СНП; среднее заполнение портов 55 % (45–62 % по сёлам) — типично для сельской плотности застройки (~2 ДХ на муфту по проекту).'),
    ('Дроп-линия', 'От РОР к каждому ДХ — индивидуальный 2-волоконный дроп-кабель (1 рабочее + 1 резервное волокно). Участок «РОР → точка подключения ДХ» прокладывается дроп-кабелем по трассе магистрали: средняя подводка +31 м на ДХ (максимум 100 м), суммарно 71,4 км. Максимальная суммарная дроп-линия — 201 м.'),
    ('Сплиттеры 1-й ступени', 'PLC 1×8 в ОРШ: 66 шт на 6 СНП (по каждому селу — сплиттеры 2-й ступени / 8, округление вверх); каждый питает до 8 РОР.'),
    ...
    ('Число волокон на участке', 'Сплиттеры 2-й ступени ниже по потоку × 1,25 (резерв 25 % на развитие), минимум 8 волокон (минимальная серийная ёмкость). Верхний участок — 204 волокна (Верхнеберезовка, до 3 параллельных кабелей у ОРШ); кабели ёмкостью более 48 волокон в каскадной схеме требуются только вблизи ОРШ крупных сёл.'),
    ('Запас на монтаж', '+10 % к расчётной длине магистрали и +5 % к дроп-кабелю; округление вверх до 0,1 км.'),
    ('Справочно', f'Суммарная расчётная ёмкость магистрали по шести СНП — {T["fiber_km"]:,.0f} волокно-км (в централизованной схеме — 4 375 волокно-км, −71 %).'.replace(',', ' ')),
    ('S', '4. Узлы и монтажные нормы', None),
    ('Муфты', 'Только узлы ветвления магистрали (115 шт по шести СНП): участки без ветвлений выполняются неразрезным кабелем, транзитные волокна не сращиваются. РОР учитываются отдельной позицией; смена ёмкости кабеля совмещается с РОР и муфтами ветвления.'),
    ('ОРШ', '1 на СНП, на общественном/административном здании в центре села (лист «Параметры сетей»). Расчётная ёмкость портов — 1,1 × волокна, уходящие к РОР, с округлением вверх до ряда 144 / 288 / 576 / 864 / 1152; каскадная схема снижает требуемую ёмкость ОРШ ~в 3,5 раза (1 008 против 3 600 портов суммарно).'),
    ('Сварные соединения', 'Оценка: пигтейли кросса ОРШ + входы сплиттеров РОР + ответвления и смены ёмкости кабеля в узлах, с запасом 10 %. Гильзы КДЗС — по числу сварок. В каскадной схеме дропы к сварке не подключаются (механические коннекторы) — объём сварок в 2 раза меньше централизованной схемы (2 585 против 5 136).'),
    ...
    ('S', '5. Прочее', None),
    ('Активное оборудование', 'Порты PON OLT — 66 (справочно; в централизованной схеме — 39): следствие каскада при заполнении портов сплиттеров менее 100 %. Модель, резервирование и комплектацию уточняет заказчик.'),
    ('Сравнение схем', 'Количественное сравнение централизованной и каскадной схем по всем позициям — лист «Сравнение схем».'),
