# -*- coding: utf-8 -*-
"""
Шаг 19. Расчёт сводной таблицы материалов FTTH по 6 СНП ВКО — КАСКАДНАЯ СХЕМА
с соотношением сплиттеров 1:4 (ОРШ) x 1:16 (РОР) = 1:64.

Архитектура: GPON, двухступенчатое каскадное деление 1:4 (ОРШ) x 1:16 (РОР) = 1:64
(та же суммарная ёмкость PON-порта, что и в централизованной схеме 1:64 и в
предыдущем варианте каскада 1:8 x 1:8).

Модель (топология сетей НЕ меняется — те же трассы, те же точки подключения):
  - РОР (бокс со сплиттером 2-й ступени 1:16) размещается в проектной муфте,
    обслуживающей группу ДХ; в кластер РОР включаются ДХ соседних муфт
    в радиусе R_MAX=150 м по трассе сети, не более CAP=16 ДХ на РОР
    (кластер <= 16 ДХ -> на РОР ровно 1 сплиттер 1:16);
  ...
  - сплиттеры 1-й ступени 1:4 — в ОРШ: ceil(N2 / 4) на СНП;
...
Параметры R_MAX=150/CAP=16/MIN_CLUSTER=5 выбраны по сетке шага 18 (колено
компромисса: число РОР -37% к варианту 1:8x1:8, заполнение портов 45%,
подводки +70 км, волокно-км -35%, сварки -40%).
"""
...
# --- параметры модели -------------------------------------------------------
SPLIT1 = 4                   # сплиттер 1-й ступени (в ОРШ)
SPLIT2 = 16                  # сплиттер 2-й ступени (в РОР)
R_MAX = 150.0                # радиус кластера РОР по трассе, м
CAP = 16                     # максимум ДХ на один РОР (= ёмкость сплиттера 2-й ступ.)
...
def main():
    # прежние расчёты — для сверки инвариантов и сравнения
    cent = json.load(open(f'{BASE}/work/boq_data.json', encoding='utf-8'))
    casc88 = json.load(open(f'{BASE}/work/boq_cascade_data.json', encoding='utf-8'))

    out = []
    print(f"{'Село':<18}{'ДХ':>5}{'РОР':>5}{'спл2':>6}{'заполн%':>8}{'ДХ/РОР':>7}{'магистр':>8}"
          f"{'вол-км':>7}{'верх':>5}{'муфты':>6}{'сварки':>7}{'дроп':>7}{'подводки':>9}{'макс.дроп':>10}")
    for v in VILLAGES:
        r = analyse(v)
        out.append(r)
        ...
        # инварианты (та же топология сетей, что в централизованной и каскадной 1:8x1:8)
        cv = next(c for c in cent['villages'] if c['key'] == v['key'])
        kv = next(c for c in casc88['villages'] if c['key'] == v['key'])
        assert r['dhx_served'] == cv['dhx_served'] == kv['dhx_served'], f"ДХ {v['key']}"
        assert r['couplers_project'] == cv['couplers'] == kv['couplers_project'], f"муфты {v['key']}"
        ...
        assert r['n2'] == r['n_pop'], f"кластер <= 16 ДХ, но сплиттеров > 1 на РОР: {v['key']}"
    print('  [инварианты: ДХ/муфты/дропы/магистраль совпадают с прежними расчётами ✓]')
    ...
    print('\nИТОГО каскад 1:4 x 1:16:', json.dumps(tot, ensure_ascii=False))
    ...
    # --- сравнение с прежними вариантами ---
    t88 = casc88['totals']
    print('\nСравнение с каскадом 1:8 x 1:8 (R=100/CAP=8):')
    for k, label in [('n_pop', 'РОР'), ('n2', 'сплиттеры 2-й ступ.'), ('splitters1', 'сплиттеры 1-й ступ. (OLT)'),
                     ('fiber_km', 'волокно-км'), ('extra_km', 'подводки, км'), ('drop_cable_km', 'дроп-кабель, км'),
                     ('splices', 'сварки'), ('mufty', 'муфты ветвления')]:
        old, new = t88.get(k), tot.get(k)
        if old is None:
            old = casc88['materials_total'].get(k if k != 'splices' else 'splices')
        d = (new / old - 1) * 100 if old else float('nan')
        print(f"  {label:<28} {old:>9} -> {new:>9}  ({d:+.1f}%)")
    ...
    json.dump(dict(model=dict(
              ...
              open(f'{BASE}/work/boq_cascade16_data.json', 'w', encoding='utf-8'),
              ensure_ascii=False, indent=1)
    print('\nСохранено: work/boq_cascade16_data.json')
