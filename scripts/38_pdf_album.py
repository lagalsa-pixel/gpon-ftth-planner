...
def title_page(c):
    ...
    subs = ('Децентрализованная структура: зонные ОРШ (сплиттеры 1:64)',
            ...
            '2 765 обслуживаемых домохозяйств · 32 зонных ОРШ + 6 ЦУ · ОРШ в центрах секторов, фидеры в границе НП')
    for s in subs:
    ...
    kpis = [('6', 'сельских населённых пунктов'),
            ('2 765', 'домохозяйств (ДХ)'),
            ('32 + 6', 'зонных ОРШ + ЦУ (OLT)'),
            ('1 474,4 км', 'суммарного волокна')]
    ...
    c.drawRightString(W - M, 52, 'ВКО · 6 СНП · 2765 ДХ')
