...
def title_page(c):
    ...
    subs = ('Децентрализованная структура: зонные ОРШ (сплиттеры 1:64)',
            ...
            '3 052 обслуживаемых домохозяйств · 36 зонных ОРШ + 6 ЦУ · ОРШ в центрах секторов, фидеры в границе НП')
    for s in subs:
    ...
    kpis = [('6', 'сельских населённых пунктов'),
            ('3 052', 'домохозяйств (ДХ)'),
            ('36 + 6', 'зонных ОРШ + ЦУ (OLT)'),
            ('1 491,1 км', 'суммарного волокна')]
    ...
    c.drawRightString(W - M, 52, 'ВКО · 6 СНП · 3052 ДХ')
