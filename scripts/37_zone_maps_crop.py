# -*- coding: utf-8 -*-
"""
Шаг 37. Рендер карт зон ОРШ (схема D) на ОБРЕЗАННЫХ кадрах пользователя.

Указание пользователя: «перенеси сети на обрезанные изображения — необходимо
использовать их». Базовые кадры — upload/02..06_*.jpg (те же, что в шагах 10/11
для карт FTTH). Топология сетей, зоны, муфты и дропы — БЕЗ ИЗМЕНЕНИЙ (как в
шаге 33); меняется только подложка и преобразование координат:
  T(p) = M_inv (шаг 08/36): px старой мозаики -> px кадра пользователя.
Кадры — двукратный апскейл (scale ~0.5), поэтому все размеры отрисовки
умножаются на k = 1/scale (та же физическая величина, что на картах шага 33).

Верхнеберезовка: кадр пользователя не предоставлен — карта шага 33
(base_restitch) остаётся без изменений.

Слои, цвета, шапка, легенда зон, условные обозначения, масштабная линейка,
стрелка севера — как в шаге 33. Отличия: легенда — автоподбор угла (BL/BR) по
минимуму перекрытых ДХ (кадры теснее мозаик); композит полосами (экономия ОЗУ
на Винном 148 Мп).

Выход: download/snp_vko/<NN>_<Село>_зоны_ОРШ_схема_D.jpg (JPG q90 4:4:4 —
замена мозаичной версии; прежние копии — work/qa/zone_maps/mosaic_versions/)
        work/qa/zone_maps/<NN>_preview_crop.png
Запуск: python3 37_zone_maps_crop.py <key>|all
"""
...
def fit_font(dr, text, sz, max_w, bold=True):
    """Шрифт с авто-уменьшением, чтобы строка влезла в max_w (кадры уже мозаик)."""
    f = fnt(sz, bold)
...
class Labels:
    """Размещение подписей без наложений (жадно, 8 кандидатов). Размеры — в px кадра."""
...
def render_village(key):
    ...
    # --- кадр пользователя и преобразование ---
    tr = json.load(open(f'{BASE}/work/{key}/crop_transform.json', encoding='utf-8'))
    Mi = tr['M_inv']                       # old mosaic px -> crop px (2x3)
    scale = tr['scale']
    k = 1.0 / scale
    ...
    W, H = base.size
    print(f'  кадр {W}x{H}, k={k:.2f}, {mpp_new:.4f} м/px', flush=True)
    ...
    mx = S(26)
    f1 = fit_font(dh, f"с. {db['name']} — схема D: зонные ОРШ (сплиттеры 1:64) при едином узле OLT",
                  S(46), W - 2 * mx)
    dh.text((mx, S(12)),
            f"с. {db['name']} — схема D: зонные ОРШ (сплиттеры 1:64) при едином узле OLT",
            font=f1, fill=(240, 245, 250))
    l2 = (f"{db['raion']} · {db['so']} · обрезанный кадр пользователя ({mpp_new:.2f} м/px, "
          f"спутник Google z18 ×{k:.0f}) · топология сети, муфты и дропы — без изменений (как в схемах A/B/C)")
    dh.text((mx, S(68)), l2, font=fit_font(dh, l2, S(27), W - 2 * mx, bold=False),
            fill=(170, 185, 200))
    ...
    dh.text((mx, S(106)), st, font=fit_font(dh, st, S(27), W - 2 * mx), fill=(255, 220, 120))

    # --- легенда зон: поиск позиции по минимуму перекрытых элементов ---
    # Кандидаты в КООРДИНАТАХ КАРТЫ (без шапки; вставка ниже с +HH):
    # BL/BR (нижние углы), ML/MR (середины боковых краёв), TL (под шапкой).
    # Веса: ДХ=1, муфта=2, зонный ОРШ=20, ЦУ=50 — подписи ОРШ закрывать нельзя.
    rows = []
    ...
    LW = S(900)
    LHH = S(58 + len(rows) * 52 + 36 + 50 + 7 * 52 + 28)
    m = S(20)
    ...
    lg_pos = (lx, ly + HH)                    # вставка в канве — со шапкой
    ...
    ld.text((S(20), S(14)), 'ЗОНЫ ОРШ (схема D, S_MIN = 15)', font=fl_t, fill=(240, 240, 240))
    yy = S(58)
    ...
    draw_dashed(ld, [(S(20), yy), (S(52), yy)], PALETTE[1] + (235,), S(7),
                dash=S(30), gap=S(22))
    ld.text((S(64), yy - S(15)), 'фидер ЦУ → зонный ОРШ (ceil(1,25 × сплиттеры зоны))',
    ...
    ld.text((S(64), yy - S(15)), 'зонный ОРШ — шкаф уличный (в муфте ветвления)',
            font=fl_r, fill=(228, 232, 238))
    yy += S(52)
    ...
    # --- масштабная линейка (в углу напротив легенды) ---
    ...
    dh.text((x0s + sb_px / 2 - S(34), y0s - S(44)), txt, font=fnt(S(27)),
            fill=(255, 255, 255))

    # --- стрелка севера ---
    nx, ny = W - S(70), HH + S(64)
    dh.polygon([(nx, ny - S(34)), (nx - S(13), ny + S(16)), (nx, ny + S(6)),
                (nx + S(13), ny + S(16))], outline=(255, 255, 255), width=S(3))
    dh.text((nx - S(8), ny + S(18)), 'С', font=fnt(S(27)), fill=(255, 255, 255))
    ...
    prev.save(f'{QAD}/{db["num"]}_preview_crop.png')
...
def main():
    ...
    for key in keys:
        assert key in CROPS, f'нет кадра пользователя для {key}'
